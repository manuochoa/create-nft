 ...
 * _Available since v3.1._
 */
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI, TokenURI, Ownable {
    ...
    constructor (string memory uri) public {
     ...
     /**
     * @dev Sets the managers to control minting
     * @param _managers array with the address of each manager
     */
    ...
    /**
     * @dev Change the managers, remove privileges of old ones and set new managers
     * @param _newManagers array with the address of each new manager
     */
    function changeManagers (address [] memory _newManagers) public onlyOwner {
        require (_newManagers.length > 0, "managers required");
    ...
    {
        uint _id = supply;
        _mint(msg.sender, _id, _amount, _metaDataURI, _metaData);
        supply++;
    }
     ...
     * acceptance magic value.
     */
    function _mint(address account, uint256 tokenId, uint256 amount, string memory tokenUri, string memory _metaData) internal virtual {
        ...
        extraInfoMap[tokenId] = TokenExtraInfo({
            ...
            metaData: _metaData,
            metaDataHash: bytes(_metaData)
        });

        _beforeTokenTransfer(operator, address(0), account, _asSingletonArray(tokenId), _asSingletonArray(amount), bytes(_metaData));
        ...
        _doSafeTransferAcceptanceCheck(operator, address(0), account, tokenId, amount, bytes(_metaData));
    }
    ...
    */
    ...
    function createBatch (address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public onlyManager{
